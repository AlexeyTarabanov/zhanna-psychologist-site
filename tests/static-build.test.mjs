...
test("okna booking popup is configured", async () => {
  const js = await readJsTree(path.join(dist, "assets"));

  assert.match(js, /https:\/\/okna\.digital\/embed\.js/);
  assert.match(js, /data-okna-booking-popup/);
  assert.match(js, /zhanna\/jeanna/);
});
